...
public class DisperseEventMonitorParams extends AbstractParams {
	...
	private String queueType=null; //"runQueue"; 				//The queue that this agent will belong to -- those with interactions should be in runQueue, while those without should be in standing
	private long normalTick=0; //7l /*day*/ * (24*60*60*1000); 								//The normal tick length when no interactions are present
	...
	public ArrayList<Calendar> getSetRunTimes(){
		...
		dates.add(TimeUtils.getNewCalendar(2005, 1, 1, 0, 0));
		dates.add(TimeUtils.getNewCalendar(2006, 1, 1, 0, 0));

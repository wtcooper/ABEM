...
public class THREDDS2LocalNetCDF {
	...
	boolean allFlorida = true;		//this pulls data from New Orleans to Bimini, and from straights to top of NCOM file in Georgia
	...
	int startHour = 0;
	int endHour = 24;
	String filename= "http://edac-dap3.northerngulfinstitute.org/thredds/dodsC/AmSeas/ncom_relo_amseas_2010072900/ncom_relo_amseas_2010072900_t096.nc"; //"c:\\work\\temp\\ncom_relo_amseas_2010072500_t084.nc" ; //"dataTest/v3d_2010051500.nc"; 
	String baseURL = "http://edac-dap3.northerngulfinstitute.org/thredds/dodsC/AmSeas/";
	...
	public THREDDS2LocalNetCDF(){
		...
		if (allFlorida){
			...
			lonMax = -78.5;
			latMin = 23.5;
			latMax = 40;	//goes to end of NCOM data

			baseOutfile = "c:\\work\\data\\NCOM_AS_AllFlorida\\"; 
		}
		else {
			timeDim = 8;
			latDim = 266;
	...
	public void IO(){
		...
		try {
			...
			while (startDate.before(endDate)) {
				...
				NetcdfFileWriteable ncFile =NetcdfFileWriteable.createNew(baseOutfile + nameURL + outDateURL + ".nc");
				...
				for (int t=0; t<24; t+=3){ //don't include 24!!!!
					...
					url = baseURL+nameURL+inDateURL+"/"+nameURL+inDateURL+timeURL; 

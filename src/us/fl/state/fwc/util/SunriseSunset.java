...
public class SunriseSunset
{
	...
	/**
	 * Static method to return a Calendar array [0] = sunrise, [1] = sunset, given the date, lat, lon,
	 * and time zone as an integer of the hour offset using standard - hours as you go west.
	 * 
	 * Uses a calendar array to keep the time zone correct.
	 */
	
	public static Calendar[]  getSunset(Calendar date, double lat, double lon){
		...
		boolean	bSunriseToday  = false;	// flag for sunrise on this date
		boolean	bSunsetToday   = false;	// flag for sunset on this date
		boolean	bSunUpAllDay   = false;	// flag for sun up all day
		boolean	bSunDownAllDay = false;	// flag for sun down all day
		boolean	bDaytime	   = false;	// flag for daytime, given 
		// hour and min in dateInput
		boolean	bSunrise = false;		// sunrise during hour checked
		boolean	bSunset  = false;		// sunset during hour checked
		boolean	bGregorian = false;		// flag for Gregorian calendar
		...
		double	dfHourRise, dfHourSet;	// hour of event: SUNUP.BAS H3
		double	dfMinRise, dfMinSet;	// minute of event: SUNUP.BAS M3
		double	dfSinLat, dfCosLat;		// sin and cos of latitude
		double	dfZenith;				// SUNUP.BAS Z: Zenith
		// Many variables in SUNUP.BAS have undocumented meanings, 
		// and so are translated rather directly to avoid confusion:
		double	dfAA1 = 0, dfAA2 = 0;	// SUNUP.BAS A(2)
		double	dfDD1 = 0, dfDD2 = 0;	// SUNUP.BAS D(2)
		...
		dfLat 		= lat;
		dfLon 		= lon;
		dfTimeZone 	= date.get(Calendar.ZONE_OFFSET)/(1000*60*60);
		...
		iDay = date.get(Calendar.DAY_OF_MONTH);
			...
			if( bSunriseToday )
			{
				...
				Calendar rise = new GregorianCalendar(iYear, iMonth, iDay, (int) dfHourRise, (int) dfMinRise);
				rise.setTimeZone(TimeZone.getTimeZone("GMT"));
				times[0] = rise;
			}
			...
			if( bSunsetToday )
			{
				Calendar set = new GregorianCalendar(iYear, iMonth, iDay, (int) dfHourSet, (int) dfMinSet);
				set.setTimeZone(TimeZone.getTimeZone("GMT"));
